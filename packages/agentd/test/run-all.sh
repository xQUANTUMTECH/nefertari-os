...
if [ "$#" -gt 0 ]; then
  ...
  TESTS=(timeline timeline-links speculate planshape workingset egress journal-chain idle cgroups inferd plan trajectories enforce smoke http ocs e2e e2e-hard redteam)
fi
