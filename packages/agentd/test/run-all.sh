...
if [ "$#" -gt 0 ]; then
  ...
  TESTS=(timeline timeline-links speculate speculate-child planshape workingset egress journal-chain dedupe leases budget context recall provenance retrieval secrets idle cgroups gatefreeze waitfor inferd mcpsocket run plan trajectories enforce smoke http console ocs everyday confined pager chat e2e e2e-hard redteam)
fi
