...
if [ "$#" -gt 0 ]; then
  ...
  TESTS=(timeline timeline-links speculate speculate-child planshape workingset egress journal-chain dedupe leases budget context recall provenance retrieval idle cgroups gatefreeze waitfor inferd mcpsocket run plan trajectories enforce smoke http ocs e2e e2e-hard redteam)
fi
